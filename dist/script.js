...
var accordion = function accordion(triggerHeaders, accordionBlocks) {
  var accordionHeading = document.querySelectorAll(triggerHeaders);
  var allBlocks = document.querySelectorAll(accordionBlocks);

  function hideAllBlocks(blocks) {
    blocks.forEach(function (block) {
      block.classList.add('animated', 'fadeOutUp'); // block.classList.remove('fadeInUp')

      block.style.display = 'none';
    });
  }

  accordionHeading.forEach(function (head, i) {
...
"use strict";
__webpack_require__.r(__webpack_exports__);
var burger = function burger(_burger, burgerMen) {
  var burgerBtn = document.querySelector(_burger);
  var burgerMenu = document.querySelector(burgerMen);
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunc = function calcFunc() {
    ...
    if (sizeBlock.value === '' || materialBlock.value === '') {
      resultBlock.textContent = 'Для расчета обязательно нужен размер и материал картины';
...
var filter = function filter() {
  ...
  var markLowers = wrapper.querySelectorAll('.lowers');
...
var forms = function forms(state) {
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      var fileName = item.files[0].name.split('.').substring(0, 7) + dots + item.files[0].name.split('.')[1];
  ...
  AllForms.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      item.parentNode.appendChild(messageStatus); //end
...
var loadStyles = function loadStyles(trigger, styles) {
  var hiddenCards = document.querySelectorAll(styles);
...
var modals = function modals() {
  ...
  function openModalByScroll(selector) {
    window.addEventListener('scroll', function () {
      if (!btnPress && window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight) {
        // сколько пролистал + сколько видие у себя на экране больше либо равно всему скролу всей страницы
  ...
  openModalByScroll('.fixed-gift'); // showPopUpByTime('.popup-consultation', 60000)
};
...
var scrolling = function scrolling(upSelector) {
  var selector = document.querySelector(upSelector);
  window.addEventListener('scroll', function () {
